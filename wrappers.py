...
class Simple115StateWrapper(gym.ObservationWrapper):
  ...
  def convert_observation(observation, fixed_positions):
    ...
    Returns:
      ...
      being controlled.
    """
    def distance_ball_pllayers(players,ball):
        return np.linalg.norm(players - np.array(ball), axis=1)

    def distance_goal_ball(ball,goal):
        return math.dist(ball, goal)

    def angle_player_ball(player_xy, player_vectors, ball):
        ball = np.array(ball)
        d = distance_ball_pllayers(player_xy, ball)
        vectors = (player_xy - ball[np.newaxis,...])
        vectors = vectors/d[...,np.newaxis]
        d = []
        for i in range(len(vectors)):
            d.append(np.clip(np.dot(player_vectors[i], vectors[i]), -1.0, 1.0))
        return np.arccos(np.array(d))
    ...
    for obs in observation:
      ...
      if obs['active'] != -1:
        active[obs['active']] = 1
        active_player = obs['left_team'][obs['active']]
      o.extend(active)
      ...
      distance_left_ball = distance_ball_pllayers(obs['left_team'], obs['ball'][:2])
      distance_right_ball = distance_ball_pllayers(obs['right_team'], obs['ball'][:2])
      o.extend(do_flatten(distance_left_ball))
      o.extend(do_flatten(distance_right_ball))

      dist_left_goal = distance_goal_ball(obs['ball'][:2], [1,0])
      dist_right_goal = distance_goal_ball(obs['ball'][:2], [-1,0])
      o.extend([dist_left_goal, dist_right_goal])

      angles = angle_player_ball(obs['left_team'], obs['left_team_direction'], obs['ball'][:2])
      o.extend(do_flatten(angles))

      dist_active_temmates = distance_ball_pllayers(obs['left_team'], active_player)
      dist_active_opponents = distance_ball_pllayers(obs['right_team'], active_player)
      o.extend(do_flatten(dist_active_temmates))
      o.extend(do_flatten(dist_active_opponents))
